...
<div class="Help">
    <div class="Help_nav">
        ...
        <div class="Help_seach">
        	<div class="left_ds">您的位置：<span>帮助中心->{{$cat_name}}</span></div>
            <form action="{{url('help/SearchArticle')}}" id="SearchArticle">
            <div class="right_ds"><span><input type="text" class="text" name="keywords"></span><a href="javascript:check_form()">搜索</a></div>
            </form>
            <script>
                function check_form(){
            ...
            @else
                <div class="content">
                    <ul class="helpList">
                        ...
                        @foreach($list as $v)
                            <li>
                                <p class="que_title left"><a href="{{url('/help/detail/'.$v->id.'?menu='.$_GET['menu'].'&cat='.$id)}}">{{$v->title}}</a></p>
                                <p class="onclick left"></p>
