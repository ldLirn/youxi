...
<div class="content">
	<div class="center_title">我的位置：{!! Breadcrumbs::render('my_buy_goods') !!}</div>
    <div class="center_box">
        @include('layouts.user_left_menu')
        <div class="center_right">
        	<div class="qq_up_top">
            	<div class="fssc"></div>
                <div class="fw_c"><span>我购买的商品</span><p>在这里你可以查询到已购买的商品的信息</p></div>
            </div>
            @include('layouts.user_search')
            <div class="buyers">
           		 <div class="record">
                     <span @if(!isset($_GET['order_status'])) class="current" @endif><a href="{{FilterManager::url('order_status', '')}}">所有订单</a></span>
                     <span @if(isset($_GET['order_status'])  && $_GET['order_status']==11) class="current" @endif><a href="{{FilterManager::url('order_status', '11')}}">等待支付</a> </span>
                            ...
                            <div class="ner">
                                ...
                                <div class="w16 left" style="width: 11%">
                                    <span class="nomargin" style="line-height:20px; margin-top:10px;">
                                        ...
                                        @elseif($v->order_status=='1')
                                            <a class="nopaycancel" href="javascript:sure({{$v->order_id}},{{$v->id}})">确认收货</a>
                                         @endif
                                    </span>
...
<script type="text/javascript">
    ...
    function sure(id,gid) {
        layer.confirm("{{trans('home.goods_sure')}}", {
            btn: ["{{trans('com.sure')}}","{{trans('com.cancel')}}"] //按钮
        }, function(){
            $.ajax({
                url: "{{url('/user/goods/sure')}}/"+id,
                data:{'_method':'delete','_token':"{{csrf_token()}}",'gid':gid},
                type: "POST",
                dataType:'json',
                success:function (data) {
                    if (data.status == 1) {
