...
<div class="content">
	<div class="center_title">我的位置：{!! Breadcrumbs::render('my_needs_order') !!}</div>
    <div class="center_box">
        @include('layouts.user_left_menu')
        <div class="center_right">
        	<div class="qq_up_top">
            	<div class="fssc"></div>
                <div class="fw_c"><span>我的求购订单</span><p>在这里你可以查询到求购订单的信息</p></div>
            </div>
            <div class="buyers">
           		 <div class="record">
                     <span @if(!isset($_GET['order_status'])) class="current" @endif><a href="{{url('/user/needsOrder')}}">所有订单</a></span>
                     <span @if(isset($_GET['order_status'])  && $_GET['order_status']==3) class="current" @endif><a href="{{url('/user/needsOrder?order_status=3')}}">交易成功</a> </span>
                     <span @if(isset($_GET['order_status']) && $_GET['order_status']==12) class="current" @endif><a href="{{url('/user/needsOrder?order_status=12')}}">交易中</a></span>
                     <span @if(isset($_GET['order_status']) && $_GET['order_status']==4) class="current" @endif><a href="{{url('/user/needsOrder?order_status=4')}}">交易取消</a></span>
                 </div>
                 <table>
               		 <thead>
                         <tr>
                                ...
                                <div class="ner">
                                    ...
                                    <span class="nomargin" style="line-height:20px; margin-top:10px;">
                                        ...
                                        @if($v->pay_status=='0' && $v->order_status!='4' && $v->order_status!='5')
                                            <a target="_blank" href="{{url('/pay_order?order_sn='.$v->order_sn)}}">立即支付</a><br>
                                        @elseif($v->order_status=='0' && $v->pay_status=='1')
                                            <a target="_blank" style="color: red">已支付</a><br>
                                        @elseif($v->order_status=='1')
                                            <a target="_blank" style="color: red">正在发货</a><br>
                                        ...
                                        @elseif($v->order_status=='1')
                                            <a class="nopaycancel" href="javascript:sure({{$v->order_id}})">确认收货</a>
                                        @endif
                                    </span>
...
<script>
    ...
    function sure(id) {
        layer.confirm("{{trans('home.goods_sure')}}", {
            btn: ["{{trans('com.sure')}}","{{trans('com.cancel')}}"] //按钮
        }, function(){
            $.ajax({
