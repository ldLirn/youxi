...
class CommonController extends Controller
{
    ...
    public function upload()
    {
        ...
        if($file -> isValid()){        //检验一下上传的文件是否有效.
            ...
            $entension = $file -> getClientOriginalExtension(); //上传文件的后缀.
                ...
                // Response::download;
                 Log::info(session('users.admin_name').'上传图片'.$img_url);
                return $img_url;
            }
    }
    ...
    //汉字转拼音
    protected function Pinyin($_String, $_Code='utf-8')
    {
    ...
    public function removeNull($data)
    {
        ...
        $new['name']= array_filter($new['name']);
     ...
     *  订单号生成
     */
    public function orderSn($type='JS')
    {
    ...
    {
        $user = User::where('id',$user_id)->select('name')->first()->toArray();
        return $user['name'];
    }
    //得到所有信息
    ...
    public function GetUsername()
    {
        ...
        $user = User::where('is_admin',0)->where('name','like','%'.$keywords.'%')->orwhere('telphone','like','%'.$keywords.'%')->Orwhere('email','like','%'.$keywords.'%')->select('id','name')->get();
        return $user;
    }
}
